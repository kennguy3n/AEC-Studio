...
### Group D — Performance + architecture (Tasks 21–24)

| Item | Status |
|---|---|
| **Task 21** — Shared-memory viewport frame transport: new `crates/aec_viewport/src/frame_buffer.rs` (`render_horizon_frame(camera, w, h, frame_index)` → CPU-side RGBA8 procedural sky+ground gradient responsive to camera pitch/yaw); `ViewportService::read_frame_buffer()` validates dims, computes a `FrameKey` (camera_hash, selection_hash=0, geometry_hash=0, viewport_w, viewport_h) and routes through either the `SurfaceManager::should_render` path (GPU adapter present) or a `fallback_last_key` comparison (no-adapter mode) so the frame index only increments when inputs actually change; bridge `viewport_read_frame_buffer()` wrapper + N-API `viewport_read_frame_buffer()` returns `Option<{ bytes, width, height, frameIndex }>` with napi `Buffer::from(Vec<u8>)` for zero-copy hand-off to V8; Electron IPC `viewport:readFrameBuffer` + preload `aec.viewport.readFrameBuffer()` + `BridgeBackend.viewportReadFrameBuffer()`; `Viewport3DHost.tsx` adds a `<canvas data-testid="design-viewport-canvas">` overlay underneath the HUD and a `paintFrameBuffer` helper that uses `ctx.createImageData(w, h)` + `data.set(bytes)` + `putImageData(image, 0, 0)` driven from the existing rAF loop, with a `lastPaintedRef` guard so the painter skips `putImageData` when `frameIndex` matches the last paint (defence-in-depth on top of the bridge's idle coalescing); tests for procedural frame opacity / horizontal-up-down camera variants / coalesced idle reads / responsive-to-camera-input, plus renderer-side ViewportContainer paint + unavailable-state guard | `DONE` |
| **Task 22** — Async N-API bridge migration: 14 heavy sync endpoints (`project_open`, `project_save`, `project_audit_sync`, `project_audit_verify`, `bim_attach_ifc`, `command_apply`/`undo`/`redo`, `project_graph_list`, `design_list_assets`, `design_list_materials`, `export_pdf`/`dxf`/`ifc`/`gltf`, `build_proposal_pack`, `deliver_build_pack`, `project_export_package`, `bim_classify`/`export_ifc`/`validate`/`diff`/`generate_schedule`/`read_schedule_rows`) converted from `pub fn` → `pub async fn` via the existing `spawn_blocking_napi` wrapper. `apps/desktop/electron/bridge.ts` `NativeApi` interface return types updated to `Promise<unknown>` so any forgotten `await` becomes a compile-time error; `adaptNative` now does `(await n.endpoint(...)) as T` per call site. JSON contracts unchanged on either side; only the `await` propagation is visible to the renderer | `DONE` |
| **Task 23** — Constrained-Delaunay tessellator: new `crates/aec_geometry/src/triangulate/cdt.rs` (~700 LoC) implementing Bowyer-Watson incremental insertion with a super-triangle bootstrap + Sloan-style constraint enforcement (flip crossing edges until each required edge appears in the triangulation) + exterior/hole classification via flood-fill across non-constraint edges seeded from super-triangle vertices and per-hole centroids; orient2d and in-circle predicates over f64 in displacement form (mm-scale inputs sit well below cancellation noise). 11 unit tests cover unit square, L-shape (concave), square with square hole, square with two holes, CW boundary normalisation, rejection of <3-vertex inputs, CCW winding on output, no super-triangle leakage | `DONE` |
| **Task 24** — Screen-space LOD selector: new `crates/aec_assets/src/lod_select.rs` adds a runtime `LodSelector { near_clip_mm }` that picks the coarsest entry in a baked `LodChain` (Phase 11 Task 21 `LodChain::aggressive_for_real_mesh`) whose projected triangle pitch fits a configurable pixel-error budget. `ScreenContext` carries camera position, object AABB, vertical FoV, viewport height and the pixel-error threshold; `LodSelector::select(&ScreenContext, &LodChain) -> u32` returns the index into the chain's level list. Algorithm: walk coarsest-first, compute `pitch_world = diameter * (1 - sqrt(ratio))`, project via `pitch_screen = pitch_world * focal_px / distance`, first level under budget wins (LOD 0 always satisfies because `ratio = 1` ⇒ `pitch_world = 0`). 10 unit tests cover close → LOD0, mid → LOD1, far → LOD2, empty chain, single-level chain, degenerate bbox, near-clip protection, zero / huge thresholds, serde roundtrip | `DONE` |

### Exit criteria (Group D)

- [x] Viewport renderer produces deterministic procedural sky+ground frames driven by camera state, with a CPU-side `Vec<u8>` RGBA8 buffer handed to JS via a zero-copy napi `Buffer`.
- [x] `frame_index` only increments when camera or viewport-size inputs change (verified by Rust unit test asserting bit-identical pixels + same index on idle reads).
- [x] All 14 migrated N-API endpoints return `Promise<...>` and are awaited end-to-end through the Electron preload + renderer.
- [x] CDT tessellator passes constraint-preservation + hole-carving tests and is plugged into the existing `aec_geometry` crate's `triangulate` module.
- [x] LOD selector deterministically picks the coarsest LOD chain entry whose projected triangle pitch fits the configured pixel-error budget, with near-clip protection against camera-inside-object explosions.
- [x] `cargo test --workspace` (2370 passing, +34 from Group D), `cargo clippy --workspace --all-targets -- -D warnings`, `cargo fmt --all --check`, `npm test --workspaces` (556 passing, +2 from Group D), `npm run -w apps/desktop type-check`, `npm run -w apps/desktop lint` all green.

### Open items (Groups E–F)

- Group B (Tasks 7–14, PR #89): SVG icon set, dark mode, micro-animations, context menus, real PBR material panel, project thumbnails, template previews, panel resize handles. *Merged.*
- Group C (Tasks 15–20, PR #90): BIM viewport integration, real spatial tree, real render output display + ETA, onboarding, draft viewport. *Merged.*
- Group D (Tasks 21–24, this PR): shared-memory viewport frames, async bridge, constrained-Delaunay tessellator, screen-space LOD selector.
